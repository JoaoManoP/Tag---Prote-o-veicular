...
const EVENT_TYPES = Object.freeze([
  'LOW_FUEL', 'BATTERY_WARNING', 'ELECTRICAL_FAULT', 'ABS_WARNING',
  'CHECK_ENGINE', 'ENGINE_TEMPERATURE', 'OIL_PRESSURE', 'BRAKE_WARNING',
  'TIRE_PRESSURE'
]);
...
function createDiagnosticEvent(value = {}) {
  ...
  if (!SEVERITIES.includes(value.severity)) throw new TypeError('Severidade de diagnóstico inválida.');
...
class VehicleDiagnosticProvider {
  constructor(name, source) {
    if (new.target === VehicleDiagnosticProvider) throw new TypeError('VehicleDiagnosticProvider é abstrato.');
    if (!SOURCES.includes(source)) throw new TypeError('Origem de provider inválida.');
    this.name = name;
    this.source = source;
  }
  listEvents() { throw new Error('Provider deve implementar listEvents().'); }
}
...
class SimulationDiagnosticProvider extends VehicleDiagnosticProvider {
  ...
  listEvents(vehicleId = null) {
    return [...this.events.values()].filter(event => vehicleId == null || event.vehicleId === String(vehicleId));
  }
  setEvents(vehicleId, values = []) {
    for (const [id, event] of this.events) if (event.vehicleId === String(vehicleId)) this.events.delete(id);
  ...
  clear(vehicleId) { return this.setEvents(vehicleId, []); }
}

module.exports = { EVENT_TYPES, SEVERITIES, SOURCES, createDiagnosticEvent, VehicleDiagnosticProvider, SimulationDiagnosticProvider };
