...
function projectOnSegment(point, start, end) {
  ...
  const ax = start.longitude * scaleX,
    ay = start.latitude * scaleY,
    bx = end.longitude * scaleX,
    by = end.latitude * scaleY;
  const px = point.longitude * scaleX,
    py = point.latitude * scaleY,
    dx = bx - ax,
    dy = by - ay;
...
function matchRadarsToRoute(route, radars, options = {}) {
  ...
  const points = route.map(value =>
    Array.isArray(value) ? { latitude: Number(value[0]), longitude: Number(value[1]) } : value
  );
  return (radars || [])
    .flatMap(radar => {
      let best = null,
      ...
      for (let index = 1; index < points.length; index++) {
        const start = points[index - 1],
          ...
          projected = projectOnSegment(radar, start, end),
          distance = distanceMeters(radar, projected);
        ...
        if (!best || distance < best.distanceFromRouteMeters)
          best = {
            ...
            distanceAlongRouteMeters: travelled + segmentLength * projected.t,
            routeBearing
          };
      ...
      if (Number.isFinite(direction) && angleDifference(direction, best.routeBearing) > 70)
        return [];
      return [{ ...radar, ...best }];
    })
