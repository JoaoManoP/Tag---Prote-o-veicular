...
const accounts = [
  { name: 'Joao', email: 'joao@rastreon.demo', password: 'Joao#2026', role: 'ADMIN' },
  {
    ...
    email: 'cliente@rastreon.demo',
    password: 'Rastreon#2026',
    role: 'USER'
  },
  {
    name: 'Administrador Demonstração',
    email: 'admin@rastreon.demo',
    password: 'Admin#Rastreon2026',
    role: 'ADMIN'
  },
  {
    name: 'Desenvolvedor Demonstração',
    email: 'dev@rastreon.demo',
    password: 'Dev#Rastreon2026',
    role: 'DEVELOPER'
  }
];
...
async function seed() {
  if (process.env.NODE_ENV === 'production' && process.env.ALLOW_DEMO_ACCOUNTS !== 'true')
    throw new Error(
      'Contas de demonstração não podem ser criadas em produção sem ALLOW_DEMO_ACCOUNTS=true.'
    );
  ...
  try {
    for (const account of accounts) {
      ...
      if (existing)
        database
          .prepare('UPDATE users SET name = ?, password_hash = ?, role = ? WHERE id = ?')
  ...
  console.log(
    `Contas de demonstração preparadas: ${accounts.map(account => account.email).join(', ')}`
  );
}
