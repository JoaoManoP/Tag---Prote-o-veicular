...
class VehicleRegistryError extends Error {
  constructor(code, message, options = {}) { super(message, options); this.name = 'VehicleRegistryError'; this.code = code; }
}

class VehicleRegistryProvider {
  async lookup() { throw new VehicleRegistryError('PROVIDER_UNAVAILABLE', 'Provider de registro veicular não implementado.'); }
}
...
class GoogleGeocodingProvider {
  ...
  async reverse(latitude,longitude){const data=await requestJson(this.fetch,`${this.baseUrl}?latlng=${latitude},${longitude}&language=pt-BR&region=br&key=${encodeURIComponent(this.apiKey)}`,{},this.timeoutMs);if(!['OK','ZERO_RESULTS'].includes(data.status)||!data.results?.length)throw new Error('Endereço não encontrado');const result=data.results[0],component=type=>result.address_components?.find(item=>item.types?.includes(type))?.long_name||'';return{label:String(result.formatted_address||'Local escolhido').slice(0,240),neighborhood:component('sublocality')||component('neighborhood'),city:component('administrative_area_level_2')||component('locality'),state:component('administrative_area_level_1'),provider:'google'}}
}
