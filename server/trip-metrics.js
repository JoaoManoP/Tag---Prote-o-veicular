...
function calculateTrackMetrics(points, options = {}) {
  ...
  const source = (Array.isArray(points) ? points : [])
    .filter(validPoint)
    .map(point => ({ ...point, latitude: Number(point.latitude), longitude: Number(point.longitude), timestamp: Number(point.timestamp) }))
    .sort((first, second) => first.timestamp - second.timestamp);
  const accurate = source.filter(point => !point.suspicious && accuracyOf(point) <= maximumAccuracy);
  ...
  for (let index = 1; index < track.length; index += 1) {
    ...
    const speeds = [previous.speed, current.speed].filter(value => value !== null && value !== undefined).map(Number).filter(Number.isFinite);
    ...
    const speedMps = speeds.length ? speeds.reduce((sum, value) => sum + Math.max(0, value), 0) / speeds.length : null;
    ...
    if ((speedMps !== null && speedMps >= 0.8) || (moved && step / elapsedSeconds >= 0.8)) movingMs += elapsedMs;
