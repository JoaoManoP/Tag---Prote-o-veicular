...
const REUSABLE_IMAGE_LICENSE =
  /^(?:CC0(?:\s|$)|CC\s+BY(?:-SA)?(?:\s|$)|PUBLIC DOMAIN(?:\s|$)|PDM(?:\s|$))/i;
...
function sameNormalizedWords(left, right) {
  return (
    Boolean(normalizeWords(left)) &&
    normalizeWords(left).toLowerCase() === normalizeWords(right).toLowerCase()
  );
}
...
function titleMatchesVehicle(title, make, model) {
  ...
  return [...makeTitleAliases(make)].some(
    alias => normalizedTitle === `${alias} ${normalizedModel}`
  );
}
...
function normalizeVehicleModel(make, model, version = '') {
  ...
  const makeTokens = new Set([
    ...normalizeWords(makeOriginal).toUpperCase().split(' '),
    ...normalizeWords(makeNormalized).toUpperCase().split(' ')
  ]);
  ...
  const technical =
    /^(?:\d+(?:MT|AT|T|V|CV)?|\d+V|[A-Z]*\d+[A-Z0-9]*|FLEX|DIESEL|GASOLINA|AUT|AUTOMATICO|MANUAL|CVT|TURBO)$/i;
  const firstTechnical = modelWords.findIndex(
    (word, index) => index > 0 && (technical.test(word) || VERSION_WORDS.has(word.toUpperCase()))
  );
  const modelCoreWords = (
    firstTechnical < 0 ? modelWords : modelWords.slice(0, firstTechnical)
  ).slice(0, 4);
  ...
  while (
    originalModelWords.length &&
    makeTokens.has(normalizeWords(originalModelWords[0]).toUpperCase())
  )
    originalModelWords.shift();
  const extractedVersion =
    String(
      version || (firstTechnical >= 0 ? originalModelWords.slice(firstTechnical).join(' ') : '')
    ).trim() || null;
  return {
    makeOriginal: makeOriginal || null,
    makeNormalized: makeNormalized || null,
    modelOriginal: modelOriginal || null,
    modelNormalized: titleCase(modelCoreWords.join(' ')) || null,
...
class FalconVehicleProvider {
  ...
  async lookup(plate) {
    ...
    if (!isValidPlate(normalized))
      throw Object.assign(new Error('Placa inválida.'), { code: 'INVALID_PLATE' });
    if (!this.token)
      throw Object.assign(new Error('Falcon sem credencial.'), { code: 'PROVIDER_AUTH_ERROR' });
    let response;
    try {
      response = await this.fetch(
        `${this.baseUrl}/private/v1/vehicles/${encodeURIComponent(normalized)}/search`,
        {
      ...
      throw Object.assign(
        new Error(
          cause?.name === 'TimeoutError' ? 'Falcon excedeu o tempo limite.' : 'Falcon indisponível.'
        ),
        {
          code: cause?.name === 'TimeoutError' ? 'PROVIDER_TIMEOUT' : 'PROVIDER_UNAVAILABLE',
          cause
        }
      );
    }
    if (response.status === 401 || response.status === 403)
      throw Object.assign(new Error('Credencial Falcon rejeitada.'), {
        code: 'PROVIDER_AUTH_ERROR'
      });
    if (response.status === 404)
      throw Object.assign(new Error('Veículo não encontrado.'), { code: 'PLATE_NOT_FOUND' });
    if (response.status === 429)
      throw Object.assign(new Error('Limite Falcon atingido.'), { code: 'PROVIDER_RATE_LIMIT' });
    if (!response.ok)
      throw Object.assign(new Error('Falcon indisponível.'), { code: 'PROVIDER_UNAVAILABLE' });
    let payload;
    try {
      payload = await response.json();
    } catch {
      throw Object.assign(new Error('Resposta Falcon inválida.'), { code: 'PROVIDER_UNAVAILABLE' });
    }
    ...
    return {
      plate: normalized,
      make: data.marca || null,
      model: data.modelo || null,
      version: data.versao || null,
      manufactureYear: Number(data.ano) || null,
      modelYear: Number(data.ano_modelo) || Number(data.ano) || null,
      color: data.cor || null,
      fuel: data.combustivel || null,
      type: data.tipo || data.tipo_veiculo || null,
      provider: 'falcon'
    };
  }
}
...
class TrustCarImageProvider {
  constructor({
    baseUrl = 'https://carapi.trustcar.info/getImage',
    fetchImpl = fetch,
    timeoutMs = 7000
  } = {}) {
  ...
  async search({ make, model, year, version }) {
    ...
    try {
      ...
      throw Object.assign(new Error('Imagem indisponível.'), {
    ...
    if (
      requestedVersion &&
      returnedVersion &&
      !sameNormalizedWords(returnedVersion, requestedVersion)
    )
      return null;
    ...
    if (
      !source ||
      !sameNormalizedWords(source, 'wikimedia') ||
      !reusableLicense(license) ||
      !attribution
    )
      return null;
    ...
    if (
      attributionSubject !== attribution &&
      !titleMatchesVehicle(attributionSubject, requestedMake, requestedModel)
    )
      return null;

    let imageUrl;
    try {
      imageUrl = new URL(data.image_url);
    } catch {
      return null;
    }
...
class VehicleLookupService {
  constructor({
    ...
    vehicleProvider,
    imageProvider,
    lookupTtlMs = 30 * 86400000,
    imageTtlMs = 90 * 86400000,
    logger = console
  } = {}) {
  ...
  imageIdentity(vehicle) {
    const normalized = normalizeVehicleModel(
    ...
    return {
      ...
      year:
        normalizeYear(vehicle?.modelYear) ||
        normalizeYear(vehicle?.year) ||
        normalizeYear(vehicle?.manufactureYear)
    };
  }
  ...
  async resolveImage(vehicle) {
    ...
    const cached = this.database
      .prepare('SELECT * FROM vehicle_image_cache WHERE cache_key=? AND expires_at>?')
      .get(key, now);
  ...
  async lookup(input) {
    ...
    if (!isValidPlate(plate))
      throw Object.assign(new Error('Confira a placa informada.'), { code: 'INVALID_PLATE' });
  ...
  async refreshCachedImage(row) {
    ...
    if (imageJson !== row.image_json)
      this.database
        .prepare('UPDATE vehicle_lookup_cache SET image_json=? WHERE plate=?')
        .run(imageJson, row.plate);
    return vehicle;
  }
  ...
  async lookupOnce(plate) {
    const now = Date.now();
    const cached = this.database
      .prepare('SELECT * FROM vehicle_lookup_cache WHERE plate=?')
      .get(plate);
    ...
    try {
      ...
      if (cached)
        return { cached: true, stale: true, vehicle: await this.refreshCachedImage(cached) };
      throw error;
    }
    ...
    this.database
      ...
      .run(
        ...
        vehicle.type,
        vehicle.provider,
        JSON.stringify(vehicle.image),
        now,
        now,
        now + this.lookupTtlMs
      );
  ...
  fromCache(row) {
    let image;
    try {
      image = JSON.parse(row.image_json || 'null');
    } catch {
      image = null;
    }
    return {
