...
class OsrmRouteProvider extends RouteProvider {
  ...
  async calculate(from, to) {
    ...
    const response = await fetch(url, { headers: { 'User-Agent': 'RastroTack/1.1 (local educational demo)', Accept: 'application/json' }, redirect: 'error', signal: AbortSignal.timeout(this.timeoutMs) });
