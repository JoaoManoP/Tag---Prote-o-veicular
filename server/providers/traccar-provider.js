...
class TraccarProvider extends TrackerDeviceProvider {
  ...
  hashExternalId(value) {
    if (this.deviceHashSecret.length < 24) throw new Error('TRACCAR_DEVICE_HASH_SECRET não configurado.');
  ...
  normalize(payload = {}) {
    ...
    const timestamp = Date.parse(position.fixTime || position.deviceTime || position.serverTime || '') || Number(position.timestamp);
    ...
    return {
      ...
      eventKey: String(position.id || payload.event?.id || `${externalId}:${timestamp}`).slice(0, 180),
      point: {
        latitude,
        longitude,
        accuracy,
        ...
        timestamp,
        ignition: typeof position.attributes?.ignition === 'boolean' ? position.attributes.ignition : null,
        battery: numberInRange(position.attributes?.batteryLevel ?? position.attributes?.battery, 0, 100),
        network: typeof position.network === 'object' ? String(position.network.radioType || '') || null : null,
