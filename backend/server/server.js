...
const { createDatabase, createSessionStore } = require('../../database/database');
const {
  normalizeEmail,
  validatePassword,
...
function createApplication(options = {}) {
  ...
  app.use('/videos', express.static(path.join(publicDir, 'videos'), staticAssetOptions));
  app.use(
    '/vendor/leaflet',
    express.static(path.join(projectRoot, 'node_modules', 'leaflet', 'dist'), staticAssetOptions)
  );
  app.use(
    '/vendor/maplibre',
    express.static(
      path.join(projectRoot, 'node_modules', 'maplibre-gl', 'dist'),
      staticAssetOptions
    )
  );
  app.use(
    '/vendor/mapbox',
    express.static(path.join(projectRoot, 'node_modules', 'mapbox-gl', 'dist'), staticAssetOptions)
  );
  app.use(
    '/vendor/three',
  ...
  app.use(
    '/vendor/zxing',
    express.static(
      path.join(projectRoot, 'node_modules', '@zxing', 'browser', 'umd'),
      staticAssetOptions
    )
  );
