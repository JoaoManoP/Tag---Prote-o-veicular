...
const SUBSCRIPTION_PLANS = new Set(['rastreio', 'inteligente', 'familia']);
...
function validateRegistration(body) {
  ...
  const plan =
    typeof body?.plan === 'string' && SUBSCRIPTION_PLANS.has(body.plan) ? body.plan : 'inteligente';
  const errors = [];
  ...
  if (!validatePassword(password))
    errors.push('A senha deve ter de 8 a 72 caracteres, com letra e número.');
  return { valid: errors.length === 0, errors, data: { name, email, phone, password, plan } };
}
