...
const ux = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'ux.js'), 'utf8');
const css = fs.readFileSync(path.join(root, 'frontend', 'web', 'css', 'style.css'), 'utf8');
const refreshCss = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'css', 'dashboard-refresh.css'),
  'utf8'
);
const navigationCss = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'css', 'navigation-redesign.css'),
  'utf8'
);
const designSystem = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'css', 'design-system.css'),
  'utf8'
);
const navigationRedesign = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'js', 'navigation-redesign.js'),
  'utf8'
);
const diagnostics = fs.readFileSync(
  path.join(root, 'backend', 'server', 'vehicle-diagnostics.js'),
  'utf8'
);
const dashboard = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'dashboard.js'), 'utf8');
const mapService = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'js', 'map-service.js'),
  'utf8'
);
const navigationState = fs.readFileSync(
  path.join(root, 'frontend', 'web', 'js', 'navigation-state.js'),
  'utf8'
);
const home = fs.readFileSync(path.join(root, 'frontend', 'web', 'home.html'), 'utf8');
const register = fs.readFileSync(path.join(root, 'frontend', 'web', 'register.html'), 'utf8');
const authClient = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'auth.js'), 'utf8');
const convoyClient = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'convoy.js'), 'utf8');
...
test('mapa mantém ferramentas na lateral e celular possui QR com fallback manual', () => {
  const mobile = fs.readFileSync(path.join(root, 'frontend', 'web', 'mobile.html'), 'utf8'),
    mobileScript = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'mobile.js'), 'utf8'),
    pair = fs.readFileSync(path.join(root, 'frontend', 'web', 'pair.html'), 'utf8'),
    pairScript = fs.readFileSync(path.join(root, 'frontend', 'web', 'js', 'pair.js'), 'utf8');
...
test('locais são automáticos e garagem é separada da visão operacional', () => {
  ...
  assert.doesNotMatch(ux, /setNativePoiVisibility/);
  assert.match(mapService, /_restoreNativePoiVisibility/);
  assert.match(compactMapService, /setLayoutProperty\(layer\.id,'visibility','visible'\)/);
