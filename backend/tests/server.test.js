...
test('preferências de tour e diagnósticos simulados são persistidos separadamente', async t => {
  ...
  await agent
    .put('/api/vehicle-health/simulation')
    .send({
      vehicleId: saved.body.vehicle.id,
      events: [{ type: 'ABS_WARNING', severity: 'WARNING', detectedAt: 100 }]
    })
    .expect(200);
  const health = await agent
    .get(`/api/vehicle-health?vehicleId=${saved.body.vehicle.id}`)
    .expect(200);
...
test('Casa e Trabalho são salvos sem expor coordenadas na resposta', async t => {
  ...
  await register(agent).expect(201);
  const saved = await agent
    .put('/api/saved-places/home')
    .send({ address: 'Rua das Flores, Timóteo - MG', latitude: -19.58, longitude: -42.64 })
    .expect(200);
...
test('cerco personalizado aceita polígono e pode ser pausado', async t => {
  ...
  const saved = await agent
    ...
    .expect(201);
  const points = [
    { latitude: -19.58, longitude: -42.64 },
    { latitude: -19.58, longitude: -42.63 },
    { latitude: -19.57, longitude: -42.63 }
  ];
  const created = await agent
    .post(`/api/vehicles/${saved.body.vehicle.id}/polygon-geofences`)
    .send({ name: 'Área personalizada', points })
    .expect(201);
  assert.equal(created.body.geofence.type, 'polygon');
  await agent
    .patch(`/api/geofences/${created.body.geofence.id}/status`)
    .send({ enabled: false })
    .expect(200);
  const listed = await agent.get(`/api/vehicles/${saved.body.vehicle.id}/geofences`).expect(200);
  assert.equal(listed.body.geofences[0].enabled, false);
  assert.equal(
    classifyPolygonPosition(
      { latitude: -19.579, longitude: -42.635, accuracy: 5 },
      { centerLat: -19.577, centerLng: -42.633, points }
    ).state,
    'inside'
  );
});
...
function register(agent, overrides = {}) {
...
test('health check confirma API e banco', async t => {
  const { app } = setup(t);
  const response = await request(app).get('/api/health').expect(200);
  assert.equal(response.body.database, 'connected');
});
test('CSP permite assets HTTP na rede local e exige upgrade em produção', async t => {
  const development = setup(t, { enforceHttpsResources: false });
  const devResponse = await request(development.app).get('/').expect(200);
  assert.doesNotMatch(
    devResponse.headers['content-security-policy'] || '',
    /upgrade-insecure-requests/
  );
  const production = setup(t, { enforceHttpsResources: true });
  const prodResponse = await request(production.app).get('/').expect(200);
  assert.match(prodResponse.headers['content-security-policy'] || '', /upgrade-insecure-requests/);
});
test('CSP permite os recursos necessários dos mapas sem liberar origens genéricas', async t => {
  const { app } = setup(t);
  const response = await request(app).get('/dashboard').expect(302);
  const policy = response.headers['content-security-policy'] || '';
  assert.match(policy, /connect-src[^;]*https:\/\/tile\.openstreetmap\.org/);
  assert.match(policy, /https:\/\/api\.mapbox\.com/);
  assert.match(policy, /https:\/\/\*\.tiles\.mapbox\.com/);
  assert.match(policy, /https:\/\/events\.mapbox\.com/);
  assert.doesNotMatch(policy, /connect-src[^;]*\shttps:\s/);
});
test('configuração do mapa preserva o estilo vetorial Mapbox para todo o sistema', async t => {
  const previous = {
  ...
  const { app } = setup(t);
  const response = await request(app).get('/map-config.js').expect(200);
  assert.match(response.text, /"provider":"mapbox"/);
  assert.match(response.text, /"mapStyleUrl":"mapbox:\/\/styles\/mapbox\/navigation-day-v1"/);
...
test('todas as respostas recebem identificador de requisição rastreável', async t => {
  const { app } = setup(t);
  ...
  const supplied = await request(app)
    ...
    .expect(200);
  assert.equal(supplied.headers['x-request-id'], 'teste-request-123');
});
test('readiness e áreas restritas aplicam papéis no backend', async t => {
  const { app, database } = setup(t);
  ...
  await user.get('/api/lab/info').expect(403);
  database.prepare("UPDATE users SET role='ADMIN' WHERE email=?").run('teste@example.com');
  await user.get('/api/admin/overview').expect(200);
  database.prepare("UPDATE users SET role='DEVELOPER' WHERE email=?").run('teste@example.com');
  await user.get('/api/lab/info').expect(200);
  await user
    .post('/api/lab/telemetry/validate')
    .send({
...
test('cadastro cria usuário, hash e sessão autenticada', async t => {
  const { app, database } = setup(t);
  const agent = request.agent(app);
  await register(agent).expect(201);
  const row = database.prepare('SELECT email, password_hash FROM users').get();
...
test('cadastro persiste o plano escolhido e ignora plano inválido', async t => {
  const { app, database } = setup(t);
  const family = request.agent(app);
  const response = await register(family, { email: 'familia@example.com', plan: 'familia' }).expect(
    201
  );
  assert.equal(response.body.subscription.plan, 'familia');
  assert.equal(
    database
      .prepare('SELECT subscription_plan AS plan FROM users WHERE email = ?')
      .get('familia@example.com').plan,
    'familia'
  );
  const fallback = request.agent(app);
  await register(fallback, { email: 'fallback@example.com', plan: 'premium-inexistente' }).expect(
    201
  );
  assert.equal(
    database
      .prepare('SELECT subscription_plan AS plan FROM users WHERE email = ?')
      .get('fallback@example.com').plan,
    'inteligente'
  );
});
test('cadastro rejeita entrada inválida e e-mail duplicado', async t => {
  const { app } = setup(t);
  const agent = request.agent(app);
  await register(agent, { password: 'fraca' }).expect(400);
  await register(agent).expect(201);
  await request(app)
    .post('/api/auth/register')
    .send({ name: 'Outro', email: 'TESTE@example.com', password: 'Outra123' })
    .expect(409);
});
test('login rejeita senha incorreta e aceita credenciais corretas', async t => {
  const { app } = setup(t);
  await register(request.agent(app)).expect(201);
  await request(app)
    .post('/api/auth/login')
    .send({ email: 'teste@example.com', password: 'Errada123' })
    .expect(401);
  const agent = request.agent(app);
  await agent
    .post('/api/auth/login')
...
test('logout revoga sessão', async t => {
  const { app } = setup(t);
  const agent = request.agent(app);
  await register(agent).expect(201);
...
test('troca de senha exige CSRF e revoga as outras sessões', async t => {
  const { app } = setup(t),
    primary = request.agent(app),
    other = request.agent(app);
  await register(primary).expect(201);
  await other
    .post('/api/auth/login')
    .send({ email: 'teste@example.com', password: 'Senha123' })
    .expect(200);
  await primary
    ...
    .expect(403);
  const csrf = (await primary.get('/api/auth/csrf').expect(200)).body.token;
  await primary
    .put('/api/auth/password')
  ...
  await primary
    ...
    .send({ currentPassword: 'Senha123', newPassword: 'NovaSenha456' })
    .expect(204);
  await primary.get('/api/auth/me').expect(200);
  ...
  await request(app)
    .post('/api/auth/login')
    .send({ email: 'teste@example.com', password: 'NovaSenha456' })
    .expect(200);
});
test('home é pública, mas painel e API negam usuário não autenticado', async t => {
  const { app } = setup(t);
  await request(app).get('/').expect(200);
  await request(app).get('/dashboard').expect(302).expect('Location', '/login.html');
  await request(app).get('/api/vehicles/reference').expect(401);
  await request(app).post('/api/sessions').send({ vehicle }).expect(401);
});
test('sessão de rastreamento é persistida e isolada por proprietário', async t => {
  const { app, database } = setup(t);
...
test('pareamento manual é autenticado, cria credencial própria e aceita uso único', async t => {
  ...
  await register(owner).expect(201);
  const created = await owner.post('/api/sessions').send({ vehicle }).expect(201);
  assert.match(created.body.pairingCode, /^[A-Z0-9]{8}$/);
  const resolved = await owner
    .get(`/api/pairings/resolve?code=${created.body.pairingCode}`)
    .expect(200);
  const paired = await owner
    .post(`/api/pairings/${resolved.body.pairing.id}/confirm`)
    .send({ name: 'Android Chrome' })
    .expect(201);
  assert.equal(paired.body.sessionId, created.body.id);
  assert.ok(paired.body.credential);
  assert.notEqual(
    database.prepare('SELECT credential_hash AS hash FROM devices').get().hash,
    paired.body.credential
  );
...
test('outro usuário não reivindica pareamento nem dispositivo', async t => {
  const { app } = setup(t),
    ...
    other = request.agent(app);
  await register(owner).expect(201);
  await register(other, { email: 'outro@example.com' }).expect(201);
  const created = await owner.post('/api/sessions').send({ vehicle }).expect(201);
  await other.get(`/api/pairings/resolve?code=${created.body.pairingCode}`).expect(403);
});
test('pareamento expirado e cancelado não pode ser confirmado', async t => {
  const { app, database } = setup(t),
    owner = request.agent(app);
  await register(owner).expect(201);
  const expired = await owner.post('/api/sessions').send({ vehicle }).expect(201);
  database
    .prepare('UPDATE pairing_sessions SET expires_at=? WHERE id=?')
    .run(Date.now() - 1, expired.body.pairingId);
  await owner.get(`/api/pairings/resolve?code=${expired.body.pairingCode}`).expect(410);
  const cancelled = await owner.post('/api/sessions').send({ vehicle }).expect(201);
  await owner.delete(`/api/pairings/${cancelled.body.pairingId}`).expect(204);
  await owner.post(`/api/pairings/${cancelled.body.pairingId}/confirm`).expect(409);
});
test('valida coordenadas, posições e perfil de veículo', () => {
  assert.equal(safePosition({ latitude: 91, longitude: 0 }), null);
  assert.equal(safePosition({ latitude: '-23.5', longitude: '-46.6', accuracy: 12 }).accuracy, 12);
  assert.deepEqual(validCoordPair('-42.5,-19.4'), [-42.5, -19.4]);
  assert.equal(validCoordPair('x,20'), null);
});
test('corredor de POIs aceita até trinta coordenadas válidas', () => {
  assert.deepEqual(parsePoiRoute('-42.5,-19.4;-43,-20'), [
    [-42.5, -19.4],
    [-43, -20]
  ]);
  assert.equal(parsePoiRoute('x,-19'), null);
  assert.equal(parsePoiRoute(Array.from({ length: 31 }, () => '-42,-19').join(';')), null);
});
...
test('titular exporta somente os próprios dados sem credenciais ou tokens', async t => {
  ...
  await other
    .post('/api/vehicles')
    .send({ ...vehicle, plate: 'XYZ9Z99', type: 'car', dataSource: 'manual' })
    .expect(201);
  const exported = await owner.get('/api/privacy/export').expect(200);
  assert.match(exported.headers['content-disposition'], /^attachment;/);
  assert.equal(exported.body.user.email, 'teste@example.com');
  assert.equal(exported.body.vehicles.length, 1);
  assert.equal(exported.body.vehicles[0].plate, 'ABC1D23');
  const serialized = JSON.stringify(exported.body);
  assert.doesNotMatch(serialized, /password|tokenHash|mobile_token|auth_sessions|Senha123/i);
  assert.doesNotMatch(serialized, /XYZ9Z99/);
});

test('exclusão de conta exige CSRF, senha e frase e remove dados em cascata', async t => {
  const { app, database } = setup(t),
    agent = request.agent(app);
  await register(agent).expect(201);
  await agent
  ...
  const csrf = (await agent.get('/api/auth/csrf').expect(200)).body.token,
    del = body => agent.delete('/api/privacy/account').set('X-CSRF-Token', csrf).send(body);
  await del({ password: 'errada', confirmation: 'EXCLUIR MINHA CONTA' }).expect(401);
  await del({ password: 'Senha123', confirmation: 'excluir' }).expect(400);
  await del({ password: 'Senha123', confirmation: 'EXCLUIR MINHA CONTA' }).expect(204);
  assert.equal(database.prepare('SELECT COUNT(*) AS total FROM users').get().total, 0);
  assert.equal(database.prepare('SELECT COUNT(*) AS total FROM vehicles').get().total, 0);
  const audit = database
    .prepare("SELECT actor_user_id,action FROM audit_events WHERE action='ACCOUNT_DELETED'")
    .get();
  assert.equal(audit.actor_user_id, null);
  await agent.get('/api/auth/me').expect(401);
});

test('retenção remove apenas sessões antigas encerradas ou expiradas', async t => {
  const { database } = setup(t),
    now = Date.now();
  database
    .prepare(
      "INSERT INTO users (name,email,password_hash,created_at) VALUES ('U','u@e.com','hash',?)"
    )
    .run(now);
  const insert = database.prepare(
    'INSERT INTO tracking_sessions (id,user_id,created_at,expires_at,closed_at) VALUES (?,1,?,?,?)'
  );
  insert.run('old-closed', now - 100 * 86400000, now - 99 * 86400000, now - 99 * 86400000);
  insert.run('old-open', now - 100 * 86400000, now + 86400000, null);
  insert.run('recent-closed', now - 5 * 86400000, now + 86400000, now - 4 * 86400000);
  const result = applyDataRetention(database, { now, days: 90 });
  assert.equal(result.deletedSessions, 1);
  assert.deepEqual(
    database
      .prepare('SELECT id FROM tracking_sessions ORDER BY id')
      .all()
      .map(row => row.id),
    ['old-open', 'recent-closed']
  );
});
...
test('produção exige segredo forte, HTTPS e chaves dos providers externos', () => {
  assert.throws(
    () =>
      validateProductionConfig(
        { NODE_ENV: 'production', PUBLIC_URL: 'http://exemplo.test' },
        { sessionSecret: 'curto' }
      ),
    /SESSION_SECRET.*HTTPS/
  );
  assert.throws(
    () =>
      validateProductionConfig(
        { NODE_ENV: 'production', PUBLIC_URL: 'https://exemplo.test', ROUTE_PROVIDER: 'google' },
  ...
  assert.throws(
    () =>
      validateProductionConfig(
        { NODE_ENV: 'production', PUBLIC_URL: 'https://exemplo.test', MAP_PROVIDER: 'mapbox' },
        { sessionSecret: 'x'.repeat(32) }
      ),
    /MAPBOX_ACCESS_TOKEN/
  );
...
test('veículos possuem CRUD persistente e seleção por proprietário', async t => {
  ...
  await owner
    ...
    .send({ ...vehicle, nickname: 'Carro atualizado', type: 'car', dataSource: 'manual' })
    .expect(200);
...
test('consumo separa trechos urbano e rodoviário e retorna faixa transparente', () => {
  const result = estimateConsumption({
    ...
    urbanShare: 0.4,
    urbanKmPerLiter: 10,
    highwayKmPerLiter: 15,
    idleMilliseconds: 3600000,
    fuelPrice: 6,
  ...
  assert.equal(
    estimateConsumption({ distanceMeters: 1000, urbanKmPerLiter: 0, highwayKmPerLiter: 10 }),
    null
  );
});
...
test('providers normalizam geocodificação e rota OSRM com mocks', async () => {
  const geocoder = new NominatimGeocodingProvider({
    ...
    fetchImpl: async () => ({
      ok: true,
      json: async () => [
        { display_name: 'Timóteo, MG', lat: '-19.58', lon: '-42.64', type: 'city' }
      ]
    })
  });
  ...
  const result = await router.calculateRoute({
    origin: { latitude: -19.58, longitude: -42.64 },
    destination: { latitude: -19.4, longitude: -42.5 }
  });
...
test('Photon normaliza resultados GeoJSON sem exigir chave', async () => {
  let requestedUrl;
  const geocoder = new PhotonGeocodingProvider({
    fetchImpl: async url => {
      requestedUrl = url;
      return {
        ok: true,
        json: async () => ({
          features: [
            {
              geometry: { coordinates: [-42.64, -19.58] },
              properties: {
                name: 'Timóteo',
                state: 'Minas Gerais',
                country: 'Brasil',
...
test('Mapbox normaliza busca direta e reversa usando Geocoding v6', async () => {
  ...
  const feature = {
    ...
    properties: {
      feature_type: 'place',
      name: 'Timóteo',
      full_address: 'Timóteo, Minas Gerais, Brasil',
      coordinates: { longitude: -42.64, latitude: -19.58 },
      context: { place: { name: 'Timóteo' }, region: { name: 'Minas Gerais' } }
    }
  };
...
test('consulta por placa normaliza somente dados públicos do veículo', async () => {
  ...
  assert.deepEqual(
    ...
    { plate: 'ABC1D23', brand: 'Fiat', model: 'Argo', year: 2024, fuel: 'Flex' }
  );
  assert.equal('renavam' in result, false);
  assert.equal('chassis' in result, false);
...
test('consulta por placa aceita padrões antigo e Mercosul e mantém campos ausentes nulos', async () => {
  const calls = [];
  const provider = new PlateLookupProvider({
    token: 'segredo-do-provider',
    fetchImpl: async (_url, options) => {
      calls.push(options);
      return { ok: true, status: 200, json: async () => ({ marca: 'Ford', modelo: 'Ka' }) };
    }
  });
  const old = await provider.lookup('abc-1234'),
    mercosul = await provider.lookup('abc-1d23');
  assert.equal(old.plate, 'ABC1234');
  assert.equal(mercosul.plate, 'ABC1D23');
  assert.equal(old.version, null);
  assert.equal(old.engine, null);
  assert.equal(old.transmission, null);
  assert.doesNotMatch(JSON.stringify(old), /segredo-do-provider/);
  assert.equal(calls[0].headers.Authorization, 'Bearer segredo-do-provider');
  await assert.rejects(
    () => provider.lookup('placa ruim'),
    error => error.code === 'INVALID_PLATE'
  );
});
...
test('API Placas usa GET com credencial na URL e normaliza campos maiúsculos', async () => {
  ...
  assert.deepEqual(
    ...
    { brand: 'VW', model: 'CROSSFOX', year: 2007, fuel: 'FLEX', provider: 'apiplacas' }
  );
...
test('provider de placa padroniza autenticação, limite, timeout, offline e não encontrado', async () => {
  ...
  for (const [status, code] of statusCode) {
    const provider = new PlateLookupProvider({
      token: 'token',
  ...
  const timeout = new PlateLookupProvider({
    token: 'token',
    fetchImpl: async () => {
  ...
  await assert.rejects(
    () => timeout.lookup('ABC1D23'),
    error => error.code === 'PROVIDER_TIMEOUT'
  );
  const offline = new PlateLookupProvider({
    token: 'token',
    fetchImpl: async () => {
  ...
  await assert.rejects(
    () => offline.lookup('ABC1D23'),
    error => error.code === 'PROVIDER_UNAVAILABLE'
  );
  await assert.rejects(
    () => new PlateLookupProvider().lookup('ABC1D23'),
    error => error.code === 'PROVIDER_AUTH_ERROR'
  );
});
...
test('API de placa retorna códigos estáveis e não bloqueia preenchimento manual', async t => {
  const provider = {
    lookup: async () => {
      throw new VehicleRegistryError('PROVIDER_TIMEOUT', 'timeout');
    }
  };
  const { app } = setup(t, { plateLookupProvider: provider }),
    agent = request.agent(app);
  await register(agent).expect(201);
  const invalid = await agent.get('/api/vehicles/lookup?plate=123').expect(400);
  assert.equal(invalid.body.code, 'INVALID_PLATE');
  assert.equal(invalid.body.retryable, false);
  const timeout = await agent.get('/api/vehicles/lookup?plate=ABC1D23').expect(502);
  assert.equal(timeout.body.code, 'PROVIDER_TIMEOUT');
  assert.equal(timeout.body.retryable, true);
});

test('preço de combustível fica separado do veículo, possui fonte e isolamento', async t => {
  const { app, database } = setup(t),
    owner = request.agent(app),
    other = request.agent(app);
  await register(owner).expect(201);
  ...
  const saved = await owner
    ...
    .send({ ...vehicle, type: 'car', dataSource: 'manual' })
    .expect(201);
  assert.equal('price' in saved.body.vehicle, false);
  assert.equal(
    database.prepare('SELECT fuel_price FROM vehicles WHERE id=?').get(saved.body.vehicle.id)
      .fuel_price,
    0
  );
  await owner.get('/api/fuel-price?fuelType=Gasolina').expect(200, { preference: null });
  const updated = await owner
    .put('/api/fuel-price')
    .send({ fuelType: 'Gasolina', pricePerLiter: 6.49, region: 'Vale do Aço' })
    .expect(200);
  assert.equal(updated.body.preference.source, 'user-provided');
  assert.equal(updated.body.preference.pricePerLiter, 6.49);
  const own = await owner.get('/api/fuel-price?fuelType=Gasolina').expect(200);
  assert.equal(own.body.preference.region, 'Vale do Aço');
  await other.get('/api/fuel-price?fuelType=Gasolina').expect(200, { preference: null });
  await owner.put('/api/fuel-price').send({ fuelType: 'Gasolina', pricePerLiter: 0 }).expect(400);
});

test('provider FIPE valida código e escolhe o ano correspondente', async () => {
  const provider = new FipePriceProvider({
    fetchImpl: async () => ({
      ok: true,
      json: async () => [
        {
  ...
  const price = await provider.lookup('001004-9', { year: 1999 });
  assert.equal(price.value, 'R$ 11.000,00');
  assert.equal(price.modelYear, 1999);
  assert.equal(price.provider, 'brasilapi-fipe');
  await assert.rejects(() => provider.lookup('invalido'), /Código FIPE inválido/);
});

test('provider Auto.dev valida VIN, autentica no header e normaliza fotos', async () => {
  let captured;
  const provider = new AutoDevVehicleImageProvider({
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          data: {
            retail: ['https://cdn.example/1.webp'],
            wholesale: [{ url: 'https://cdn.example/2.jpg' }]
          }
        })
      };
    }
...
test('API FIPE é autenticada e mantém aviso de valor referencial', async t => {
  const fipePriceProvider = {
      lookup: async (code, { year }) => ({
        code,
        modelYear: year,
        value: 'R$ 50.000,00',
    ...
    { app } = setup(t, { fipePriceProvider }),
    agent = request.agent(app);
  await request(app).get('/api/fipe/001004-9').expect(401);
  await register(agent).expect(201);
  const response = await agent.get('/api/fipe/001004-9?year=2020').expect(200);
  assert.equal(response.body.price.value, 'R$ 50.000,00');
  assert.match(response.body.disclaimer, /referência/);
  await agent.get('/api/fipe/invalido').expect(400);
});
...
test('catálogo viário filtra por raio e categoria sem devolver a base inteira', t => {
  ...
  database
    .prepare(
      'INSERT INTO road_events (fingerprint,category,label,longitude,latitude,speed_limit,source,imported_at) VALUES (?,?,?,?,?,?,?,?)'
    )
    .run('near', 'speed_camera', 'Radar 60', -42.64, -19.58, 60, 'test', now);
  database
    .prepare(
      'INSERT INTO road_events (fingerprint,category,label,longitude,latitude,speed_limit,source,imported_at) VALUES (?,?,?,?,?,?,?,?)'
    )
    .run('far', 'speed_bump', 'Lombada', -43.64, -20.58, null, 'test', now);
  const events = new RoadEventService(database).nearby({
    latitude: -19.58,
    longitude: -42.64,
    radiusMeters: 2000,
    categories: ['speed_camera']
  });
  assert.equal(events.length, 1);
  assert.equal(events[0].speedLimit, 60);
  assert.equal(categoryFor('Pedágio - 0 kmh'), 'toll');
});

test('Google Routes mantém chave no header e normaliza ETA com trânsito', async () => {
  let captured;
  const router = new GoogleRouteProvider({
    apiKey: 'segredo-teste',
    fetchImpl: async (url, options) => {
      captured = { url, options };
      return {
...
test('providers rejeitam timeout HTTP, resposta malformada e ausência de rota', async () => {
  ...
  await assert.rejects(
    () =>
      failed.calculateRoute({
        origin: { latitude: 0, longitude: 0 },
        destination: { latitude: 1, longitude: 1 }
      }),
    /504/
  );
  const malformed = new NominatimGeocodingProvider({
    baseUrl: 'https://nominatim.internal',
    fetchImpl: async () => ({ ok: true, json: async () => ({ invalid: true }) })
  });
  await assert.rejects(() => malformed.search('consulta'), /inválida/);
  const noRoute = new OsrmRouteProvider({
    fetchImpl: async () => ({ ok: true, json: async () => ({ code: 'NoRoute', routes: [] }) })
  });
  await assert.rejects(
    () =>
      noRoute.calculateRoute({
        origin: { latitude: 0, longitude: 0 },
        destination: { latitude: 1, longitude: 1 }
      }),
    /Nenhuma rota/
...
test('viagem persiste rota planejada separada do percurso GPS e calcula comparação', async t => {
  ...
  const agent = request.agent(app);
  await register(agent).expect(201);
  const savedVehicle = await agent
    .post('/api/vehicles')
    .send({ ...vehicle, type: 'car', dataSource: 'manual' })
    .expect(201);
  const tracking = await agent
    .post('/api/sessions')
    .send({ vehicleId: savedVehicle.body.vehicle.id })
    .expect(201);
  const plannedRoute = {
    routeId: 'principal',
    distanceMeters: 2000,
  ...
  const trip = await agent
    .post('/api/trips')
    .send({
      trackingSessionId: tracking.body.id,
      vehicleId: savedVehicle.body.vehicle.id,
      plannedRoute,
      startedAt: 1000
    })
    .expect(201);
  database
    .prepare(
      'INSERT INTO positions (tracking_session_id, device_id, latitude, longitude, accuracy, captured_at, received_at, source, sequence_number, accuracy_class) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    )
    .run(
      tracking.body.id,
      'test-device',
...
test('histórico de viagens é isolado por usuário', async t => {
  ...
  await stranger
    .patch(`/api/trips/${created.body.trip.id}/finish`)
    .send({ endedAt: Date.now() })
    .expect(404);
...
test('persistência de posição é idempotente por sessão e sequence', t => {
  ...
  const userId = database
    ...
    .run('Teste', 'idempotente@example.com', 'hash', Date.now()).lastInsertRowid;
  database
    .prepare('INSERT INTO tracking_sessions (id, user_id, created_at) VALUES (?, ?, ?)')
    .run('tracking-idempotente', userId, Date.now());
  const point = safePosition({
    latitude: -19.5,
    longitude: -42.5,
    accuracy: 10,
    timestamp: 1234,
  ...
  assert.equal(
    database
      .prepare('SELECT COUNT(*) AS total FROM positions WHERE tracking_session_id = ?')
      .get('tracking-idempotente').total,
    1
  );
});
...
test('reconstrução classifica candidatos com componentes e pesos transparentes', () => {
  ...
  const ranked = rankReconstructionCandidates({
    routes,
    gapDurationSeconds: 620,
    speedBefore: 10,
    speedAfter: 9,
    headingBefore: 45,
    headingAfter: 45,
    plannedGeometry: routes[0].geometry
  });
...
test('API persiste lacuna e alternativas sem alterar posições GPS originais', async t => {
  ...
  const response = await agent
    .post(`/api/trips/${trip.body.trip.id}/reconstruct`)
    .send({
      before: {
        latitude: -19.5,
        longitude: -42.6,
  ...
  assert.equal(
    database.prepare('SELECT COUNT(*) AS total FROM reconstruction_candidates').get().total,
    2
  );
  assert.equal(
    database.prepare('SELECT COUNT(*) AS total FROM positions').get().total,
    beforeCount
  );
});
...
test('horários e alertas são persistidos e isolados por proprietário', async t => {
  ...
  const saved = await owner
    ...
    .send({ ...vehicle, type: 'car', dataSource: 'manual' })
    .expect(201);
  const id = saved.body.vehicle.id,
    schedule = {
      enabled: true,
      days: [1, 2, 3, 4, 5],
      from: '22:00',
  ...
  const ownerId = database
    ...
    .get('teste@example.com').id;
  database
    .prepare(
      'INSERT INTO alerts (id, user_id, vehicle_id, type, severity, title, occurred_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    )
    .run(
      'alerta-owner',
      ownerId,
  ...
  const ownerAlerts = await owner.get('/api/alerts').expect(200),
    strangerAlerts = await stranger.get('/api/alerts').expect(200);
...
test('geofence circular distingue dentro, fora, borda e precisão ruim', () => {
  ...
  assert.equal(
    classifyCirclePosition({ latitude: 0, longitude: 0, accuracy: 5 }, fence).state,
    'inside'
  );
  assert.equal(
    classifyCirclePosition({ latitude: 0.002, longitude: 0, accuracy: 5 }, fence).state,
    'outside'
  );
  assert.equal(
    classifyCirclePosition({ latitude: 0.0009, longitude: 0, accuracy: 15 }, fence).state,
    'uncertain'
  );
  assert.equal(
    classifyCirclePosition({ latitude: 0.002, longitude: 0, accuracy: 120 }, fence).state,
    'uncertain'
  );
});
...
test('CRUD de geofence pertence exclusivamente ao dono do veículo', async t => {
  ...
  const created = await owner
    .post(`/api/vehicles/${saved.body.vehicle.id}/geofences`)
    .send(payload)
    .expect(201);
...
test('cenário offline demonstrativo persiste pontos em ordem e interrupção', async t => {
  ...
  const tracking = await agent
    .post('/api/sessions')
    .send({ vehicleId: saved.body.vehicle.id })
    .expect(201);
  const points = [3, 1, 2].map(sequence => ({
    latitude: -19.5 + sequence / 1000,
    longitude: -42.6,
    accuracy: 8,
    speed: 10,
  ...
  const response = await agent
    ...
    .send({ sessionId: tracking.body.id, points, lostAt: 1000, reconnectedAt: 3000 })
    .expect(201);
  assert.equal(response.body.received, 3);
  assert.match(response.body.disclaimer, /demonstrativo/);
  assert.deepEqual(
    database
      .prepare('SELECT sequence_number AS sequence FROM positions ORDER BY captured_at')
      .all()
