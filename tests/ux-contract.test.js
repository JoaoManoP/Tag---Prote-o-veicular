...
const homeHeroCss = fs.readFileSync(
  path.join(root, 'public', 'css', 'home-clean-hero.css'),
  'utf8'
);
...
test('home carrega novamente o vídeo limpo do carro como fundo', () => {
  ...
  assert.doesNotMatch(home, /poster=/);
  assert.doesNotMatch(homeHeroCss, /home-hero-night\.png/);
});

test('cabeçalho da home usa a logo clara completa em tamanho legível', () => {
  assert.match(home, /rastreon-logo\.png\?v=20260829-header/);
  assert.doesNotMatch(home, /rastreon-mark-dark\.png/);
  assert.match(homeHeroCss, /\.site-header \.brand img/);
  assert.match(homeHeroCss, /width: 150px !important/);
});
