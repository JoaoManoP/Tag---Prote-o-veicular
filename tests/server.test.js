...
test('configuração do mapa preserva o estilo vetorial Mapbox para todo o sistema', async (t) => { const previous={MAP_PROVIDER:process.env.MAP_PROVIDER,MAP_STYLE_URL:process.env.MAP_STYLE_URL,MAPBOX_ACCESS_TOKEN:process.env.MAPBOX_ACCESS_TOKEN};process.env.MAP_PROVIDER='mapbox';process.env.MAP_STYLE_URL='mapbox://styles/mapbox/navigation-day-v1';process.env.MAPBOX_ACCESS_TOKEN='pk.test-token';t.after(()=>{for(const [key,value] of Object.entries(previous))value===undefined?delete process.env[key]:process.env[key]=value});const { app }=setup(t);const response=await request(app).get('/map-config.js').expect(200);assert.match(response.text,/"provider":"mapbox"/);assert.match(response.text,/"mapStyleUrl":"mapbox:\/\/styles\/mapbox\/navigation-day-v1"/);assert.match(response.text,/"mapboxAccessToken":"pk\.test-token"/);assert.doesNotMatch(response.text,/"type":"raster"/);assert.doesNotMatch(response.text,/tile\.openstreetmap\.org/);});
