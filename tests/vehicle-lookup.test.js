...
test('normalizador preserva originais e simplifica fabricante e modelo', () => {
  assert.deepEqual(normalizeVehicleModel('GM', 'ONIX 1.0MT LT'), {
    makeOriginal: 'GM',
    makeNormalized: 'Chevrolet',
    modelOriginal: 'ONIX 1.0MT LT',
    modelNormalized: 'Onix',
    version: '1.0MT LT'
  });
  assert.deepEqual(normalizeVehicleModel('VW', 'GOL 1.0 MI'), {
    makeOriginal: 'VW',
    makeNormalized: 'Volkswagen',
    modelOriginal: 'GOL 1.0 MI',
    modelNormalized: 'Gol',
    version: '1.0 MI'
  });
  assert.deepEqual(normalizeVehicleModel('GM/CHEVROLET', 'ONIX PLUS PREMIER 1.0 TURBO'), {
    makeOriginal: 'GM/CHEVROLET',
    makeNormalized: 'Chevrolet',
    modelOriginal: 'ONIX PLUS PREMIER 1.0 TURBO',
    modelNormalized: 'Onix Plus',
    version: 'PREMIER 1.0 TURBO'
  });
  assert.deepEqual(normalizeVehicleModel('PEUGEOT', '208 ALLURE 1.6'), {
    makeOriginal: 'PEUGEOT',
    makeNormalized: 'Peugeot',
    modelOriginal: '208 ALLURE 1.6',
    modelNormalized: '208',
    version: 'ALLURE 1.6'
  });
});
...
test('Falcon usa rota fixa, Bearer e resposta veicular normalizada', async () => {
  let captured;
  const provider = new FalconVehicleProvider({
    token: 'segredo',
    baseUrl: 'https://falcon.example/data-hub',
...
test('TrustCar consulta marca, modelo, ano e versão e preserva a licença', async () => {
  let requestedUrl;
  const valid = new TrustCarImageProvider({
    fetchImpl: async url => {
...
test('TrustCar rejeita imagem sem correspondência exata ou licença reutilizável', async t => {
  ...
  const search = async (
    overrides,
    query = { make: 'Chevrolet', model: 'Onix Plus', year: 2022 }
  ) => {
    const provider = new TrustCarImageProvider({
      fetchImpl: async () => ({ ok: true, json: async () => ({ ...base, ...overrides }) })
    });
      ...
      await search(
        {
          ...
          model: 'Vectra',
          title: 'Opel Vectra',
          attribution: 'Opel Vectra — © Autor (CC BY-SA 4.0)'
        },
        { make: 'Chevrolet', model: 'Vectra', year: 2022 }
      ),
      null
    );
  });
  ...
  await t.test('versão divergente quando informada pelo provedor', async () => {
    assert.equal(
      await search(
        { version: 'Premier' },
        { make: 'Chevrolet', model: 'Onix Plus', year: 2022, version: 'LT' }
      ),
      null
    );
  });
...
test('serviço deduplica consulta e reutiliza caches persistentes', async () => {
  ...
  const service = new VehicleLookupService({
    ...
    imageProvider: {
      search: async () => {
        ...
        return {
          found: true,
          url: 'https://cdn.trustcar.info/onix.jpg',
          source: 'wikimedia',
          license: 'CC',
          author: null,
          attribution: null,
  ...
  assert.equal(first.vehicle.make, 'Chevrolet');
  assert.equal(same.vehicle.model, 'Onix');
  assert.equal(vehicleCalls, 1);
  assert.equal(imageCalls, 1);
  database
    .prepare("UPDATE vehicle_lookup_cache SET image_json = ? WHERE plate = 'ABC1D23'")
    .run(JSON.stringify({ found: true, url: 'https://example.invalid/wrong-model.jpg' }));
  const cached = await service.lookup('ABC1D23');
  assert.equal(cached.cached, true);
  assert.equal(cached.vehicle.image.url, 'https://cdn.trustcar.info/onix.jpg');
  assert.equal(vehicleCalls, 1);
  assert.equal(imageCalls, 1);
  database.close();
});
...
test('serviço não degrada para modelo abreviado nem remove o ano', async () => {
  ...
  assert.ok(
    attempts.every(
      attempt =>
        attempt.model === 'Onix Plus' &&
        attempt.year === 2022 &&
        attempt.version === 'PREMIER 1.0 TURBO'
    )
  );
  database.close();
});
...
test('cache de imagem separa versões do mesmo modelo e ano', async () => {
  ...
  const service = new VehicleLookupService({
    ...
    imageProvider: {
      search: async () => ({
        found: true,
        url: `https://cdn.trustcar.info/${++calls}.jpg`,
        source: 'wikimedia',
        license: 'CC BY 4.0',
        author: 'Autor',
        attribution: 'Crédito',
  ...
  const lt = await service.resolveImage({
    make: 'Chevrolet',
    model: 'Onix',
    version: 'LT',
    modelYear: 2022
  });
  const premier = await service.resolveImage({
    make: 'Chevrolet',
    model: 'Onix',
    version: 'Premier',
    modelYear: 2022
  ...
  assert.equal(
    database.prepare('SELECT COUNT(*) AS total FROM vehicle_image_cache').get().total,
    2
  );
  database.close();
});
