...
const { normalizeVehicleKey, resolveVehicleImage } = require('../public/js/vehicle-images');
...
test('aceita somente caminho local para futura foto do proprietário', () => {
  assert.equal(resolveVehicleImage({ customImageUrl: '/uploads/my-car.webp' }).source, 'owner-upload');
  assert.equal(resolveVehicleImage({ customImageUrl: 'javascript:alert(1)' }).source, 'unavailable');
  assert.equal(resolveVehicleImage({ customImageUrl: 'https://example.com/car.jpg' }).source, 'unavailable');
});

test('aceita imagens remotas somente dos provedores autorizados', () => {
  const trusted = resolveVehicleImage({ image: { url: 'https://cdn.trustcar.info/vehicles/vectra.jpg', source: 'trustcar' } });
  assert.equal(trusted.source, 'trustcar');
  assert.equal(trusted.matchedBy, 'brand-model');
  assert.equal(resolveVehicleImage({ image: { url: 'https://example.com/car.jpg' } }).source, 'unavailable');
});
