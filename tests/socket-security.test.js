...
async function setup(t) {
  ...
  const context = createApplication({
    databasePath: ':memory:',
    sessionSecret: 'socket-test-secret-with-at-least-32-chars',
    silent: true
  });
  await new Promise((resolve, reject) =>
    context.server.listen(0, '127.0.0.1', error => (error ? reject(error) : resolve()))
  );
...
function mobileInvite(url) {
  return new URLSearchParams(new URL(url).hash.slice(1));
}
async function pairDevice(agent, created) {
  const token = mobileInvite(created.body.pairUrl).get('token'),
    resolved = await agent
      .get(`/api/pairings/resolve?token=${encodeURIComponent(token)}`)
      .expect(200),
    confirmed = await agent
      .post(`/api/pairings/${resolved.body.pairing.id}/confirm`)
      .send({ name: 'Celular de teste' })
      .expect(201);
  return { token: confirmed.body.credential, deviceId: confirmed.body.device.id };
}
...
test('convite móvel exige token e não expõe histórico nem placa', async t => {
  ...
  await agent
    .post('/api/auth/register')
    .send({ name: 'Teste Socket', email: 'socket@example.com', password: 'Senha123' })
    .expect(201);
  ...
  const stored = database
    .prepare('SELECT mobile_token_hash AS hash FROM tracking_sessions WHERE id = ?')
    .get(created.body.id);
  assert.notEqual(stored.hash, token);
  assert.notEqual(stored.hash, pairingToken);
  assert.equal(stored.hash.length, 64);

  const socket = await connect(url);
  t.after(() => socket.close());
  const denied = await emit(socket, 'session:join', {
    sessionId: created.body.id,
    role: 'mobile',
    token: 'token-invalido-com-tamanho-suficiente-123456',
    deviceId
  });
  assert.equal(denied.ok, false);
  const joined = await emit(socket, 'session:join', {
    sessionId: created.body.id,
    role: 'mobile',
    token,
    deviceId
...
test('Socket.IO rejeita telemetria inválida e limita frequência ao vivo', async t => {
  ...
  await agent
    .post('/api/auth/register')
    .send({ name: 'Teste GPS', email: 'gps@example.com', password: 'Senha123' })
    .expect(201);
  const created = await agent.post('/api/sessions').send({ vehicle }).expect(201);
  const { token, deviceId } = await pairDevice(agent, created);
  const socket = await connect(url);
  ...
  assert.equal(
    (
      await emit(socket, 'session:join', {
        sessionId: created.body.id,
        role: 'mobile',
        token,
        deviceId
  ...
  assert.equal(
    (await emit(socket, 'consent:grant', { deviceId: base.deviceId, purpose: 'vehicle-tracking' }))
      .ok,
    true
  );
  ...
  const limited = await emit(socket, 'position:update', {
    ...base,
    timestamp: Date.now(),
    sequence: 2
  });
  ...
  const revoked = await emit(socket, 'position:update', {
    ...base,
...
test('API bloqueia mutação originada por outro site', async t => {
  ...
  await request(app)
    ...
    .send({ name: 'Ataque', email: 'ataque@example.com', password: 'Senha123' })
    .expect(403);
});
