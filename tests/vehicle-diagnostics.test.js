...
} = require('../server/vehicle-diagnostics');
...
test('eventos exigem tipo, severidade e origem válidos', () => {
  ...
  assert.throws(() =>
    createDiagnosticEvent({ type: 'INVENTED', severity: 'WARNING', source: 'SIMULATION' })
  );
});

test('provider de simulação nunca aceita DTC como leitura real', () => {
  const provider = new SimulationDiagnosticProvider();
  const events = provider.setEvents('vehicle-1', [
    { type: 'CHECK_ENGINE', severity: 'CRITICAL', dtc: { code: 'P0001', source: 'OBD' } }
  ]);
