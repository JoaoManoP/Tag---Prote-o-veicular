...
const databaseCriticalPatterns = [
  /^(?:backend\/)?server\/(?:backup-database|database|init-database|migrations|provision-staff|seed-demo-accounts|set-role)\.js$/,
  /^database\//,
  /^data\//,
  /^deploy\/.*(?:database|migration)/i,
  /^scripts\/.*(?:database|migration)/i
];

const runtimeGeneratedFiles = new Set(['public/js/vehicle-3d-layer.bundle.js']);
