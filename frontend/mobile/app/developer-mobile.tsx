    ...
    return (
      <Screen>
        <EmptyState
          ...
          message="Este módulo respeita a mesma função DEVELOPER do sistema."
        />
      </Screen>
    );
  return (
    <Screen>
      <Header
        title="Laboratório"
        subtitle="Integrações e telemetria"
        action={<StatusBadge status="DEVELOPER" />}
      />
      {[
        ['Ambiente', info],
        ['Feature flags', flags],
        ['Rastreadores', bindings]
      ].map(([label, value]) => (
        <Card key={label as string}>
          <Text style={{ color: theme.colors.text, fontWeight: '900' }}>{label as string}</Text>
          <Text selectable style={{ color: theme.colors.muted }}>
            {value ? JSON.stringify(value, null, 2) : 'Carregando…'}
          </Text>
        </Card>
      ))}
    </Screen>
