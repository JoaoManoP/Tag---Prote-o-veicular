...
window.RastroMap.ready
  .then(context => {
    ...
    async function loadTripHistory(selectedId) {
      ...
      if ($('historyMetricTrips')) $('historyMetricTrips').textContent = String(trips.length);
      ...
      $('historyMetricMoving').textContent = formatDuration(latest.comparison.movingSeconds);
      $('historyMetricStopped').textContent = formatDuration(latest.comparison.stoppedSeconds);
      $('historyMetricSpeed').textContent = `${br(latest.comparison.maximumSpeedKmh)} km/h`;
