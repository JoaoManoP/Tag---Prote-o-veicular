...
async function loadAdmin() {
  ...
  document.getElementById('adminUpdated').textContent = new Date(
    data.generatedAt
...
async function loadLab() {
  ...
  if (!response.ok)
    throw new Error(
      response.status === 403
        ? 'Seu perfil não possui acesso ao Laboratório.'
  ...
  document.getElementById('labPhysicalTag').textContent = data.physicalTagEnabled
    ? 'Ativada'
    : 'Desativada';
  const sendPoint = async (invalid = false) => {
    const point = {
      ...
      longitude: -43.934567,
      accuracy: Number(document.getElementById('labAccuracy').value),
      altitude: 852.3,
      altitudeAccuracy: 12,
      speed: 11.5,
      heading: 180,
      source: 'simulation',
      sequence: Number(document.getElementById('labSequence').value)
    };
    const result = await fetch('/api/lab/telemetry/validate', {
      method: 'POST',
