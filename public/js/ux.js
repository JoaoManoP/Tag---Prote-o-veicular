(function () {
  ...
  const helpByPage = {
    tracking: [['Como rastrear meu carro?', 'Crie uma sessão, conecte o celular e inicie a viagem.'], ['Por que meu veículo está offline?', 'Confira a conexão do celular usado no veículo.'], ['O que significa posição estimada?', 'É um trecho reconstruído quando o GPS ficou temporariamente indisponível.']],
  ...
  const tours = {
    tracking: [['.map-column', 'Este é o mapa. Ele permanece como foco do rastreamento.'], ['#vehicleSheet', 'Toque no resumo para ver destino, distância e saúde.'], ['#exploreBtn', 'Ative somente o tipo de local que deseja visualizar.']],
    ...
    developer: [['#healthOptions', 'Ative avisos simulados para testar a interface.'], ['.preset-row', 'Os cenários preparam combinações rapidamente.']]
  };
  ...
  const icon = name => `<svg class="ui-icon" aria-hidden="true" viewBox="0 0 24 24"><use href="/images/ui-icons.svg#${name}"></use></svg>`;
  ...
  function setupStructure() {
    ...
    if (mapCard) {
      ...
      if (!devToolsEnabled) {
        ...
        if (!byId('tripPlannerBtn')) { const trip = document.createElement('button'); trip.id = 'tripPlannerBtn'; trip.className = 'secondary'; trip.textContent = 'Viagem'; actions?.insertBefore(trip, byId('simulateBtn')); }
    ...
    document.querySelector('#destinationInput')?.setAttribute('placeholder', 'Buscar endereço ou CEP');
    const originLabel = byId('originInput')?.closest('label'); if (originLabel?.firstChild) originLabel.firstChild.textContent = 'De onde você vai sair?';
    const destinationLabel = byId('destinationInput')?.closest('label'); if (destinationLabel?.firstChild) destinationLabel.firstChild.textContent = 'Para onde você vai?';
    byId('calculateBtn').textContent = 'Calcular rota';
    byId('createBtn').textContent='Conectar celular para iniciar';
    byId('routeSummary')?.insertAdjacentHTML('beforeend', '<div class="route-cost"><span>Custo estimado</span><strong id="routeCostSummary">—</strong></div><button id="routeDetailsBtn" class="text-btn" type="button">Ver detalhes</button>');
  }
  ...
  function setPage(page) {
    ...
    renderHelp();
    if (page === 'developer') {
      document.querySelectorAll('.view').forEach(v => v.classList.toggle('active', v.id === 'developerView'));
      document.querySelectorAll('.nav-pill').forEach(n => n.classList.toggle('active', n.dataset.view === page));
    }
  ...
  async function loadHealthTimeline() {
    try {
      const response = await fetch('/api/vehicle-health'), data = await response.json();
      const timeline = byId('eventTimeline'); if (!timeline || !data.events?.length) return;
      const existing = timeline.querySelector('.health-history'); existing?.remove();
      const section = document.createElement('section'); section.className = 'health-history';
  ...
  async function searchAddress(query, targetId, onSelect) {
    const target = byId(targetId); if (query.trim().length < 3) { target.classList.add('hidden'); return; }
    target.innerHTML = '<span class="searching">Buscando endereço…</span>'; target.classList.remove('hidden');
    try {
      const response = await fetch(`/api/geocode?q=${encodeURIComponent(query.trim())}`), places = await response.json();
  ...
  function chooseFencePoint(point, label) {
    ...
    byId('fenceAddress').textContent = label; byId('fenceChosen').classList.remove('hidden'); byId('activateFence').disabled = false;
    const api=window.rastreonMap; if (!api) return;
    if (fencePreview) api.map.removeLayer(fencePreview);
    const radius = Number(document.querySelector('[name="fenceSize"]:checked')?.value || byId('fenceCustomSize').value);
    fencePreview=api.L.circle([fencePoint.latitude,fencePoint.longitude],{radius,color:'#ff5a0a',fillColor:'#ff5a0a',fillOpacity:.12,weight:3}).addTo(api.map).bindTooltip(`${byId('fenceName').value || 'Área'} · área protegida`).openTooltip();
    api.map.fitBounds(fencePreview.getBounds(),{padding:[45,45]});
  }

  async function reverseAddress(lat, lng) {
    const response=await fetch(`/api/reverse-geocode?lat=${lat}&lng=${lng}`), data=await response.json();
  ...
  async function activateFence() {
    ...
    const radius=Number(document.querySelector('[name="fenceSize"]:checked')?.value || byId('fenceCustomSize').value);
    const payload=customPolygon?{name:byId('fenceName').value||'Área protegida',type:'polygon',points:customPolygon,centerLat:fencePoint.latitude,centerLng:fencePoint.longitude,radiusMeters:radius,enabled:true}:{name:byId('fenceName').value||'Área protegida',type:'circle',centerLat:fencePoint.latitude,centerLng:fencePoint.longitude,radiusMeters:radius,enabled:true};
    const endpoint=customPolygon?`/api/vehicles/${selectedVehicleId}/polygon-geofences`:`/api/vehicles/${selectedVehicleId}/geofences`;const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const data=await response.json(); if(!response.ok) return showNotice(data.error);
    showNotice(`${data.geofence.name} está protegida.`); byId('fenceConfirmation').classList.add('hidden');byId('activateFence').classList.remove('hidden');customPolygon=null;loadManagedFences();
  }
  ...
  async function activateArrivalFence() { if(!arrivalPlace||!selectedVehicleId)return; fencePoint={latitude:arrivalPlace.latitude,longitude:arrivalPlace.longitude,label:arrivalPlace.address}; byId('fenceName').value=arrivalPlace.label; document.querySelector('[name="fenceSize"][value="350"]').checked=true; await activateFence(); byId('arrivalPrompt').classList.add('hidden'); }

  function showNotice(message) { const toast=byId('toast'); if(!toast)return; toast.textContent=message; toast.classList.add('show'); setTimeout(()=>toast.classList.remove('show'),2600); }

  function requestNumberComplement(input) {
    input.closest('label').querySelector('.address-complement')?.remove();
    const box=document.createElement('div');box.className='address-complement';box.innerHTML='<span>Encontramos o endereço. Qual é o número?</span><div><input inputmode="numeric" aria-label="Número do endereço"><button type="button">Confirmar</button></div>';
    box.querySelector('button').onclick=()=>{const number=box.querySelector('input').value.trim();if(number)input.value=`${input.value}, nº ${number}`;box.remove();};input.closest('label').appendChild(box);box.querySelector('input').focus();
  }

  async function savePlace(key) {
    if(!fencePoint)return showNotice('Escolha primeiro um endereço.');
    const response=await fetch(`/api/saved-places/${key}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({address:fencePoint.label,latitude:fencePoint.latitude,longitude:fencePoint.longitude})});
    if(response.ok){showNotice(`${key==='home'?'Casa':'Trabalho'} salvo.`);loadSavedPlaces();}
  }
  ...
  function renderHelp() {
    ...
    byId('helpQuestions').innerHTML = questions.map(([q,a]) => `<details><summary>${q}</summary><p>${a}</p></details>`).join('');
  }

  function showTourWelcome(page) {
    const names = {tracking:'Rastreamento',trip:'Viagem Inteligente',fence:'Cerco', timeline:'Histórico', vehicles:'Veículos', profile:'Perfil', developer:'Laboratório'};
  ...
  function finishTour(dismissed = false) {
    byId('tourWelcome').classList.add('hidden'); byId('tourTip').classList.add('hidden');
    document.querySelectorAll('.tour-target').forEach(x => x.classList.remove('tour-target'));
    localStorage.setItem(`rastreon-tour-${currentPage}`, dismissed ? 'dismissed' : 'completed');
    fetch(`/api/tour-preferences/${encodeURIComponent(currentPage)}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ completed: !dismissed, dismissed }) }).catch(() => {});
  }

  function selectedHealth() {
    return [...document.querySelectorAll('#healthOptions input:checked')].map(input => healthTypes.find(x => x[0] === input.value));
  }
  ...
  async function saveHealth() {
    const events = selectedHealth().map(([type,,severity]) => ({ type, severity: severity === 'critical' ? 'CRITICAL' : 'WARNING', estimatedValue: type === 'LOW_FUEL' ? Number(byId('simFuelLevel').value) : null, detectedAt: Date.now() }));
    try {
      const response = await fetch('/api/vehicle-health/simulation', { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ vehicleId:selectedVehicleId, events }) });
  ...
  function renderPois(places, category) {
    ...
    const zoom = api.map.getZoom(), cell = zoom >= 15 ? .002 : zoom >= 13 ? .008 : .025;
    const groups = new Map();
    for (const place of places) {
      const key = `${Math.round(place.latitude / cell)}:${Math.round(place.longitude / cell)}`;
      const group = groups.get(key) || []; group.push(place); groups.set(key, group);
    }
  ...
  async function loadPreferencesAndHealth() {
    try {
      const [vehiclesResponse, preferencesResponse] = await Promise.all([fetch('/api/vehicles'), fetch('/api/tour-preferences')]);
      const vehicles = await vehiclesResponse.json(), preferences = await preferencesResponse.json();
      selectedVehicleId = vehicles.vehicles?.find(vehicle => vehicle.selected)?.id || vehicles.vehicles?.[0]?.id || null;
      for (const preference of preferences.preferences || []) localStorage.setItem(`rastreon-tour-${preference.tourKey}`, preference.dismissed ? 'dismissed' : 'completed');
      if (selectedVehicleId) {
        const response = await fetch(`/api/vehicle-health?vehicleId=${selectedVehicleId}`), data = await response.json();
        document.querySelectorAll('#healthOptions input').forEach(input => input.checked = (data.events || []).some(event => event.type === input.value));
        const fuel = (data.events || []).find(event => event.type === 'LOW_FUEL'); if (fuel?.estimatedValue) { byId('simFuelLevel').value = fuel.estimatedValue; byId('simFuelOutput').textContent = `${fuel.estimatedValue}%`; }
        renderHealth();
      }
    } catch (_) {}
  }
  ...
  function applyPreset(name) {
    const presets = {normal:[], fuel:['LOW_FUEL'], electrical:['BATTERY_WARNING','ELECTRICAL_FAULT'], engine:['CHECK_ENGINE','ENGINE_TEMPERATURE'], multiple:['LOW_FUEL','ABS_WARNING','CHECK_ENGINE','TIRE_PRESSURE']};
    document.querySelectorAll('#healthOptions input').forEach(i => { i.checked = presets[name].includes(i.value); });
    renderHealth();
  }
  ...
  function syncSummary() {
    ...
    setText(byId('routeCostSummary'), `R$ ${byId('costMin')?.textContent || '0,00'}–${byId('costMax')?.textContent || '0,00'}`);
  }
  ...
  function bindEvents() {
    document.addEventListener('click', event => {
      ...
      if (event.target.closest('#sheetHandle')) {
        ...
        const next = sheet.classList.contains('minimized') ? 'half' : sheet.classList.contains('half') ? 'expanded' : 'minimized';
    ...
    window.rastreonSocket?.on('vehicle-health:update', payload => {
      ...
      document.querySelectorAll('#healthOptions input').forEach(input => input.checked = payload.events.some(event => event.type === input.value));
      renderHealth();
    });
    window.rastreonSocket?.on('saved-place:arrival', place=>{arrivalPlace=place;byId('arrivalTitle').textContent=`Você chegou em ${place.label}.`;byId('arrivalPrompt').classList.remove('hidden');});
    window.rastreonSocket?.on('position:update',position=>{clearTimeout(addressTimer);addressTimer=setTimeout(async()=>{try{const label=await reverseAddress(position.latitude,position.longitude);if(byId('sheetCurrentAddress'))byId('sheetCurrentAddress').textContent=label.split(',').slice(0,3).join(',');}catch(_){}},500);});
    window.rastreonMap?.map.on('moveend',()=>{clearTimeout(addressTimer);addressTimer=setTimeout(async()=>{const center=window.rastreonMap.map.getCenter();try{const label=await reverseAddress(center.lat,center.lng);const short=label.split(',').slice(0,3).join(',');if(byId('sheetCurrentAddress'))byId('sheetCurrentAddress').textContent=short;}catch(_){}},700);});
  }

  setupStructure(); bindEvents(); renderHelp(); syncSummary();
  try { const saved = JSON.parse(localStorage.getItem('rastreon-health-simulation') || '[]'); document.querySelectorAll('#healthOptions input').forEach(i => i.checked = saved.includes(i.value)); } catch (_) {}
  renderHealth();
  loadPreferencesAndHealth().finally(() => { if (!localStorage.getItem('rastreon-tour-tracking')) setTimeout(() => showTourWelcome('tracking'), 250); });
}());
