...
(function exposeNavigation() {
  ...
  const formatDistance = meters =>
    meters < 1000
      ...
      : `${(Math.max(0, meters) / 1000).toFixed(1).replace('.', ',')} km`;
  const routeDistance = (position, geometry = []) => {
    if (geometry.length < 2) return Infinity;
    const latScale = 111320,
      lngScale = latScale * Math.cos(radians(position.latitude)),
      stride = Math.max(1, Math.ceil(geometry.length / 250));
    let best = Infinity;
    for (let index = 0; index < geometry.length - 1; index += stride) {
      const a = geometry[index],
        b = geometry[Math.min(geometry.length - 1, index + stride)],
        ax = (a[1] - position.longitude) * lngScale,
        ay = (a[0] - position.latitude) * latScale,
        bx = (b[1] - position.longitude) * lngScale,
        by = (b[0] - position.latitude) * latScale,
        dx = bx - ax,
        dy = by - ay,
        t = Math.max(0, Math.min(1, -(ax * dx + ay * dy) / Math.max(1, dx * dx + dy * dy))),
        x = ax + t * dx,
        y = ay + t * dy;
  ...
  class VehiclePositionInterpolator {
    ...
    move(marker, from, to, duration = 650) {
      ...
      const element =
        marker.getElement?.()?.querySelector('.vehicle-marker-body') || marker.getElement?.();
  ...
  class NavigationStateService {
    constructor({ map, container }) {
      ...
      this.stepIndex = 0;
      this.traveled = 0;
      this.interpolator = new VehiclePositionInterpolator();
      this.voice = false;
      ...
      document.getElementById('navRecenter').onclick = () => {
        this.follow = true;
        this.lastPosition && this.center(this.lastPosition);
        document.getElementById('navRecenter').classList.remove('attention');
      };
      document.getElementById('navPerspective').onclick = event => {
        const enabled = event.currentTarget.getAttribute('aria-pressed') !== 'true';
        event.currentTarget.setAttribute('aria-pressed', String(enabled));
        this.map.setTilt?.(enabled ? 55 : 0);
      };
      document.getElementById('navVoice').onclick = event => {
        this.voice = event.currentTarget.getAttribute('aria-pressed') !== 'true';
        event.currentTarget.setAttribute('aria-pressed', String(this.voice));
        if (!this.voice) globalThis.speechSynthesis?.cancel();
      };
      document.getElementById('navRoadEvents').onclick = event => {
        const enabled = event.currentTarget.getAttribute('aria-pressed') !== 'true';
        event.currentTarget.setAttribute('aria-pressed', String(enabled));
        window.dispatchEvent(
          new CustomEvent('rastreon:road-events-toggle', { detail: { enabled } })
        );
      };
      document.getElementById('nav2d').onclick = () => {
        this.map.setTilt?.(0);
      ...
      this.map.on('dragstart', () => {
        ...
        this.follow = false;
        document.getElementById('navRecenter').classList.add('attention');
      });
    }
    setRoute(route, destination = 'Destino') {
      this.route = route;
    ...
    center(position) {
      this.map.panTo
        ...
        : this.map.setView(
            ...
            Math.max(16, this.map.getZoom())
          );
    }
    update(position, speedKmh, traveledMeters) {
      this.lastPosition = position;
      this.traveled = traveledMeters || 0;
      const steps = this.route?.steps || [];
      let step = steps[this.stepIndex];
      while (
        step?.location &&
        distance(position, step.location) < 35 &&
        this.stepIndex < steps.length - 1
      )
        step = steps[++this.stepIndex];
      if (this.voice && step && this.lastAnnouncedStep !== this.stepIndex) {
        this.lastAnnouncedStep = this.stepIndex;
        globalThis.speechSynthesis?.cancel();
        const message = new SpeechSynthesisUtterance(step.instruction || 'Continue na rota');
        message.lang = 'pt-BR';
        globalThis.speechSynthesis?.speak(message);
      }
      const deviation = routeDistance(position, this.route?.geometry);
      this.deviationCount = deviation > 120 ? this.deviationCount + 1 : 0;
      if (this.active && this.deviationCount >= 3 && Date.now() - this.lastRerouteAt > 45000) {
        this.deviationCount = 0;
        this.lastRerouteAt = Date.now();
        window.dispatchEvent(
          new CustomEvent('rastreon:route-deviation', {
            detail: { position, distanceMeters: Math.round(deviation) }
          })
        );
      }
      document.getElementById('navSpeed').textContent = Math.round(speedKmh || 0);
      if (step?.location)
        document.getElementById('navDistance').textContent = formatDistance(
          distance(position, step.location)
        );
    ...
    render() {
      ...
      const step = this.route.steps?.[this.stepIndex],
        remaining = Math.max(
          0,
          (this.route.distance || this.route.distanceMeters || 0) - this.traveled
        ),
        duration =
          (this.route.duration || this.route.durationSeconds || 0) *
          (remaining / Math.max(1, this.route.distance || this.route.distanceMeters || 1));
      ...
      document.getElementById('navInstruction').textContent =
        step?.instruction || 'Continue pela rota destacada';
      ...
      document.getElementById('navEta').textContent = new Date(
        Date.now() + duration * 1000
      ).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
      document.getElementById('navRemaining').textContent =
        `${formatDistance(remaining)} · ${Math.max(1, Math.round(duration / 60))} min`;
    }
  }
