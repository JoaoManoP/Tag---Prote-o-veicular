...
const labels: Record<string, string> = {
  completedTrips: 'Viagens',
  continuity: 'Continuidade',
  scheduleCompliance: 'Horários',
  geofenceCompliance: 'Proteção',
  dataQuality: 'GPS'
};
  ...
  return (
    <Screen>
      <Header
        ...
        title="Condução responsável"
        subtitle="Ranking somente com consentimento"
      />
      {progress ? (
        <>
          ...
          <Card>
            {Object.entries(progress.breakdown).map(([key, value]) => (
              <View key={key} style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: theme.colors.muted }}>{labels[key] || key}</Text>
          ...
          <Card>
            <Input
              ...
              onChangeText={value => setProfile(current => ({ ...current, displayName: value }))}
            />
            <Button
              secondary={profile.enabled}
              title={profile.enabled ? 'Participando do ranking' : 'Ranking desativado'}
              onPress={() => setProfile(current => ({ ...current, enabled: !current.enabled }))}
            />
            <Button
              title="Salvar preferência"
              onPress={async () => {
                await api.put('/api/gamification/me', profile);
                await load();
              }}
            />
          </Card>
          {ranking.length ? (
            ranking.map(item => (
              <Card key={`${item.position}-${item.displayName}`}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ color: theme.colors.text, fontWeight: '900' }}>
                    {item.position}º · {item.displayName}
                  </Text>
            ...
            <EmptyState
              icon="trophy-outline"
              title="Ranking vazio"
              message="A participação é opcional e não publica rotas ou localização."
            />
          )}
        </>
      ) : (
        <EmptyState
          ...
          message="Usando viagens e telemetria do mesmo sistema."
        />
      )}
    </Screen>
  );
