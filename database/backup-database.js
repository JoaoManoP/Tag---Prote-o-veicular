...
function backupDatabase({
  databasePath = process.env.DATABASE_PATH || path.join(__dirname, 'data', 'rastreon.sqlite'),
  backupDirectory = process.env.BACKUP_DIRECTORY || path.join(__dirname, 'backups'),
  now = new Date()
} = {}) {
  const source = path.resolve(databasePath),
    directory = path.resolve(backupDirectory);
  ...
  const destination = path.join(directory, `rastreon-${safeTimestamp(now)}.sqlite`),
  ...
  try {
    ...
    if (integrity.integrity_check !== 'ok')
      throw new Error('A cópia falhou na verificação de integridade.');
    verify.prepare('SELECT 1').get();
  } finally {
    verify.close();
  }
  return destination;
